...
from vnpy_pro.trader.utility import ArrayManagerPro
...
class TwineSignal(CtaSignal):
    ...
    def __init__(
        ...
        self.am = self.am_x_min = ArrayManagerPro(5)
...
class TwineStrategy(CtaTemplatePro):
    """
    """
    author = "jin"
    ...
    def on_tick(self, tick: TickData):
        super().on_tick(tick)
        # self.twine_signal.bg_min.update_tick(tick)
        # self.renko.update_tick(tick)

    def on_bar(self, bar: BarData):
        super().on_bar(bar)
        self.twine_signal.bg_min.update_bar(bar)
        # self.renko.update_bar(bar)
